...
import {
  ...
  ShieldAlert, Truck, MapPin, Users, Clock, Activity, ShoppingBag,
  QrCode, LogOut,
} from "lucide-react";
...
export function AttendeeDashboard() {
  ...
  const quickActions = [
    ...
    { icon: QrCode, label: "QR Identity", description: "Child safety tags", route: "/qr-identity", color: "bg-[#06b6d4]", glow: "shadow-[0_0_15px_rgba(6,182,212,0.2)]" },
    { icon: Truck, label: "Logistics", description: "Request delivery", route: "/logistics", color: "bg-[#10b981]", glow: "shadow-[0_0_15px_rgba(16,185,129,0.2)]" },
  ];
      ...
      <div className="bg-[#1a1f2e]/80 backdrop-blur-lg border-b border-white/10 p-4 sticky top-0 z-10">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-white/60 capitalize">{userProfile?.role?.replace("_", " ") ?? "Attendee"}</p>
          </div>
          ...
          {[
            ...
            <div key={label} className="mb-3 last:mb-0">
              <div className="flex items-center justify-between text-sm mb-1.5">
                ...
                <span className={`text-xs ${percent > 75 ? "text-red-400" : percent > 50 ? "text-amber-400" : "text-[#10b981]"}`}>{level} — {percent}%</span>
              </div>
