...
function NotifItem({ notif, onRead, onDelete }: {
  ...
  return (
    <div
      className={`flex items-start gap-3 p-3 rounded-lg transition-all ${notif.read ? 'opacity-60' : 'bg-white/3'} hover:bg-white/5`}
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <p className={`text-sm ${notif.read ? 'text-white/50' : 'text-white'} leading-tight`}>{notif.title}</p>
  ...
  return (
    <div className="relative">
      ...
      >
        ...
        {unreadCount > 0 && (
          <span className="absolute -top-0.5 -right-0.5 min-w-[18px] h-[18px] rounded-full bg-red-500 text-white text-[10px] flex items-center justify-center px-1 font-bold">
      ...
      {open && (
        <>
          ...
          <div className="absolute right-0 top-full mt-2 w-80 z-50 rounded-xl bg-[#1a1f2e] border border-white/10 shadow-2xl shadow-black/50 overflow-hidden">
            <div className="flex items-center justify-between px-4 py-3 border-b border-white/10">
              ...
              {unreadCount > 0 && (
                <button
                  ...
                  className="flex items-center gap-1 text-xs text-[#0ea5e9] hover:text-[#0ea5e9]/80 transition-colors"
                >
