  ...
  return (
    <div className="bg-white/5 border border-white/10 rounded-lg px-4 py-3 flex items-center gap-3">
      ...
      <div className="flex-1">
        ...
        <div className="text-xs text-white/60 mt-1">Server status monitored in real-time</div>
      </div>
    </div>
  );
}
...
export function WebSocketStatus() {
  ...
  return (
    <div className="flex items-center gap-2 text-sm">
      <div className="relative">
        <Zap className={`w-4 h-4 ${isConnected ? 'text-emerald-400' : 'text-white/40'}`} />
        {isConnected && (
          <div className="absolute -top-1 -right-1 w-2 h-2 bg-emerald-400 rounded-full animate-pulse" />
        )}
      </div>
      <span className="text-white/60">
        {isConnected ? (
          <>
            ...
            <span className="text-white/40 ml-2">({messageCount} msgs)</span>
          </>
        ) : (
      ...
      <div className="space-y-3">
        {services.map((service) => (
          <div key={service.name} className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              <span className="text-sm text-white/60">{service.uptime}% uptime</span>
